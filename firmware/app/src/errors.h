...
#define LIFT_ERROR_STALL (LIFT_ERROR_FIRST_ - 2)   /* Lift stationary when it should be moving or the hall sensors are not working correctly. */
#define LIFT_ERROR_DRIVER (LIFT_ERROR_FIRST_ - 3)  /* I/O error communicating with the motor driver. */
#define LIFT_ERROR_TIMEOUT (LIFT_ERROR_FIRST_ - 4) /* Operation timed out because poll wasn't called often enough. */

// BLUE
#define SPAN_ERROR_FIRST_ (-224)
...
#define SPAN_ERROR_NOT_HOME (SPAN_ERROR_FIRST_ - 2)   /* The actuators did not stall at their home position as expected. */
#define SPAN_ERROR_NOT_TRAVEL (SPAN_ERROR_FIRST_ - 3) /* The actuators traveled further than the expected distance without stalling. */
#define SPAN_ERROR_DRIVER (SPAN_ERROR_FIRST_ - 4)     /* I/O error communicating with the motor driver. */
#define SPAN_ERROR_TIMEOUT (SPAN_ERROR_FIRST_ - 5)    /* Operation timed out because poll wasn't called often enough. */
